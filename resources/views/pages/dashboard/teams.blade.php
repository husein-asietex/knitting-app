...
new #[Layout('layouts::app')] #[Title('Teams')] class extends Component{
    ...
    public function mount(){
       $this->dispatch('page-title', title: 'Teams', subtitle: 'Data untuk Teams');
    }
    ...
    public function with(): array
    {
        $teams = Teams::query()
            ...
            ->when($this->teamFilter, fn($q) => $q->where('id', $this->teamFilter))
            ->orderBy('id')
    ...
    public function save(): void
    {
        ...
        if ($this->editingId) {
            Teams::findOrFail($this->editingId)->update($data);
            $this->dispatch('notify', type: 'success', message: 'Team berhasil diperbarui.');
        } else {
            Teams::create($data);
            $this->dispatch('notify', type: 'success', message: 'Team baru berhasil ditambahkan.');
        }
    ...
    public function delete(): void
    {
        ...
        Teams::findOrFail($this->deleteId)->delete();
        $this->showDeleteModal = false;
        $this->deleteId = null;
        $this->dispatch('notify', type: 'success', message: 'Team berhasil dihapus.');
    }
                        ...
                        <td>
                            <div class="flex items-center gap-2.5">
                                <div class="w-7 h-7 rounded-full flex items-center justify-center text-white text-[11px] font-bold shrink-0"
                                    style="background:var(--navy)">{{ $team->initials() }}</div>
